  ...
  return (
    <div className="animate-in fade-in duration-500">
      <div className="flex items-center justify-between mb-8">
        <h2 className="font-serif italic text-3xl text-foreground">Записи & Конспекты</h2>
        {isEditing && (
      ...
      {!notes || notes.length === 0 ? (
        <div className="text-center py-20 border border-dashed border-border rounded-sm bg-card/50">
          ...
          <p className="font-serif italic text-muted-foreground text-lg">The journal is empty.</p>
        </div>
      ) : (
            ...
            <Link key={note.id} href={`/notes/${note.id}`} className="group block">
              <article className="h-full bg-card notebook-border p-6 hover:bg-muted/30 hover:shadow-md transition-all relative overflow-hidden">
                ...
                <p className="font-serif italic text-muted-foreground line-clamp-3 leading-relaxed">
                  {note.content?.replace(/<[^>]*>?/gm, '') || "No content written yet."}
                </p>
      ...
      <Modal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} title="New Journal Entry">
        <NoteForm onSuccess={() => setIsAddModalOpen(false)} />
      </Modal>
    </div>
  );
}
